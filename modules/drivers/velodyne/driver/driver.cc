...
VelodyneDriver* VelodyneDriverFactory::create_driver(const VelodyneConf& velodyne_conf) {
  // use private node handle to get parameters
  Config config;
  config.frame_id = velodyne_conf.frame_id();
  config.model = velodyne_conf.model();
  config.firing_data_port = velodyne_conf.firing_data_port();
  config.positioning_data_port = velodyne_conf.positioning_data_port();
  config.rpm = velodyne_conf.rpm();
