...
Compensator::Compensator(ros::NodeHandle node, const VelodyneConf& velodyne_conf)
  ...
  child_frame_id_ = velodyne_conf.compensator_child_frame_id();
  queue_size_ = 10;
  tf_timeout_ = velodyne_conf.tf_query_timeout();
}

sensor_msgs::PointCloud2::Ptr Compensator::pointcloud_callback(
...
inline bool Compensator::check_message(
  ...
  for (size_t i = 0; i < msg->fields.size(); ++i) {
    ...
    if (f.name == "x") {
      ...
      if (timestamp_offset_ == -1 || timestamp_data_size_ == (uint)-1) {
        return false;
      }
    } else {
  ...
  if (x_offset_ == -1 || y_offset_ == -1 || z_offset_ == -1 ||
      timestamp_offset_ == -1 || timestamp_data_size_ == (uint)-1) {
    return false;
  }
